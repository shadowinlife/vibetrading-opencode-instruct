...
1. Confirm frequency, target symbols, notification channels, and signal thresholds.
2. Add task with `python cron_jobs/manage.py add`.
3. Validate with `python cron_jobs/manage.py list`.
4. Run once with `python cron_jobs/manage.py run <task_id>`.
5. Install or update crontab using generated entries if required.

## CLI Examples

```bash
python cron_jobs/manage.py list
python cron_jobs/manage.py add --name "601777 close scan" --cron "30 15 * * 1-5" --prompt "Analyze 601777.SH signals" --email "user@example.com"
...
- DingTalk webhook URL is stored per task in `registry.json`.
- SMTP host, username, password/auth code, and sender are read from environment or `.env`.
- Do not hardcode API keys, webhook secrets, or SMTP credentials in scripts.
